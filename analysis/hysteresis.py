...
def hysteresis_counts_only_subset(database,gene,FP_OFF,FP_ON,gene_index,subset_size = 10000):
    start= time.clock()
    hys_query = DSGRN.HysteresisQuery(database,gene,FP_OFF,FP_ON)
    end = time.clock()
    ...
    print("\nHysteresis and resettable bistability searches over {} reduced parameters took {} seconds.\n".format(subset_size,int(end-start)))
    sys.stdout.flush()
