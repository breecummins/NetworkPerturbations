...
for n in range(single_gene_query.number_of_reduced_parameters()):
    ...
    graph.color = lambda v : "green" if graph.mgi(v) in Q else ("red" if graph.mgi(v) in P else ( "yellow" if graph.mgi(v) in B else "white"))
    graphstr = 'digraph {' + \
   ...
   '\n' + '}\n'
    ...
    for v in graph.vertices:
        if graph.color(v) == "white":
            ...
            parameter = parametergraph.parameter(single_gene_query.database.full_parameter_index(v,graph.mgi(v),database.network.index("S")))
