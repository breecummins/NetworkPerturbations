...
for NETWORKFILE in $NETWORKDIR/*; do
	...
	if [[ $RUN_TYPE = "qsub" ]]; then
		qsub $HELPER_SCRIPT_CMD $PATH_TO_DSGRN $NETWORKFILE $PATTERNDIR $DATABASEDIR $RESULTSDIR $NETWORKID $QUERYFILE $RMDB
	elif [[ $RUN_TYPE = "sbatch" ]]; then
		sbatch $HELPER_SCRIPT_CMD $PATH_TO_DSGRN $NETWORKFILE $PATTERNDIR $DATABASEDIR $RESULTSDIR $NETWORKID $QUERYFILE $RMDB
	elif [[ $RUN_TYPE = "local" ]]; then
		. $HELPER_SCRIPT_CMD $PATH_TO_DSGRN $NETWORKFILE $PATTERNDIR $DATABASEDIR $RESULTSDIR $NETWORKID $QUERYFILE $RMDB
	fi
done
